...
import { fetchPlans, formatDuration, type PlanInfo } from "../lib/plans";
...
export function PlanPicker({ chainId, refreshKey, onSelect }: PlanPickerProps) {
  ...
  useEffect(() => {
    ...
    if (!provider) {
      setError(`No RPC configured for chain ${chainId}`);
      return;
    }
    let cancelled = false;
    fetchPlans(provider, chainId)
      .then((loaded) => {
  ...
  return (
    <section className="checkout-step">
      ...
      <p className="muted">
        Pay in USDC on {getChainName(chainId)}. Cancel anytime.
      </p>

      {!deployed && <p className="muted">Not deployed on {getChainName(chainId)} yet.</p>}
      ...
      <div className="plan-grid">
        {monthly && (
          <div className="plan-card">
            ...
            <div className="plan-card__price">
              {monthly.price} <span>USDC / month</span>
            </div>
        ...
        {yearly && (
          <div className="plan-card plan-card--highlight">
            ...
            <div className="plan-card__price">
              {yearly.price} <span>USDC / year</span>
            </div>
        ...
        {test && (
          <div className="plan-card">
            ...
            <div className="plan-card__price">
              {test.price} <span>USDC / {formatDuration(test.intervalSeconds)}</span>
            </div>
        ...
        {other.map((plan) => (
          <div className="plan-card" key={plan.id}>
            ...
            <div className="plan-card__price">
              {plan.price} <span>USDC / {formatDuration(plan.intervalSeconds)}</span>
            </div>
